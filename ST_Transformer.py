...
class SSelfAttention(nn.Module):
    ...
    def forward(self, values, keys, query):
        ...
        values = values.reshape(N, T, self.heads, self.head_dim)  #embed_size Dimensions disassembled into    
                                                                   #heads×head_dim
        ...
        energy = torch.einsum("qthd,kthd->qkth", [queries, keys])   # space  self-attention
        ...
        # better stability
        attention = torch.softmax(energy / (self.embed_size ** (1 / 2)), dim=1)  # Do softmax in K dimension, and         #1
...
class TSelfAttention(nn.Module):
    ...
    def forward(self, values, keys, query):
        ...
        # Split the embedding into self.heads different pieces
        values = values.reshape(N, T, self.heads, self.head_dim)  # embed_size Dimensions disassembled into     #heads×head_dim                                          
        keys   = keys.reshape(N, T, self.heads, self.head_dim)
        query  = query.reshape(N, T, self.heads, self.head_dim)
        values  = self.values(values)  # (N, T, heads, head_dim)
        keys    = self.keys(keys)      # (N, T, heads, head_dim)
        ...
        # with every other training example, don't be confused by einsum
        # it's just how I like doing matrix multiplication & bmm
        energy = torch.einsum("nqhd,nkhd->nqkh", [queries, keys])   # time self-attention
        # queries shape: (N, T, heads, heads_dim),
        # keys shape: (N, T, heads, heads_dim)
        # energy: (N, T, T, heads)
        # Normalize energy values similarly to seq2seq + attention
        # so that they sum to 1. Also divide by scaling factor for
        # better stability
        attention = torch.softmax(energy / (self.embed_size ** (1 / 2)), dim=2)  # Do softmax in K dimension, and  #1                                                   
...
class TTransformer(nn.Module):
    def __init__(self, embed_size, heads, time_num, dropout, forward_expansion):
        ...
        self.one_hot = One_hot_encoder(embed_size, time_num)          # temporal embedding select one-hotway or
        self.temporal_embedding = nn.Embedding(time_num, embed_size)  # temporal embedding select nn.Embedding
    ...
    def forward(self, value, key, query, t):
        ...
        D_T = self.one_hot(t, N, T)                          # temporal embeddingselect one-hotway or
        D_T = self.temporal_embedding(torch.arange(0, T))    # temporal embedding select nn.Embedding
        D_T = D_T.expand(N, T, C)

        # temporal Add embedding to query。 The original paper uses concatenated
        query = query + D_T  
        attention = self.attention(value, key, query)
        # Add skip connection, run through normalization and finally dropout
        x = self.dropout(self.norm1(attention + query))
...
class Encoder(nn.Module):
    # stack multiple layers ST-Transformer Block
    def __init__(
        self,
        embed_size,
        num_layers,
...
class STTransformer(nn.Module):
    def __init__(
        ...
        # The first convolution expands the number of channels
        ...
        self.Transformer = Transformer(
            ...
            heads, 
            time_num)    
        # # Reduce the time dimension. Example: T_dim=12 to output_T_dim=3, input 12 dimensions down to output 3    
        #dimensions
        self.conv2 = nn.Conv2d(T_dim, output_T_dim, 1)  
        # Reduce the number of channels and reduce to 1 dimension
        self.conv3 = nn.Conv2d(embed_size, 1, 1)
        self.relu = nn.ReLU()
    def forward(self, x, t):
        # input x shape[ C, N, T] 
                #C: number of channels. N: Number of sensors. T: amount of time
        x = x.unsqueeze(0)
        ...
        out = out.permute(0, 3, 2, 1) #left of the equal sign out shape: [1, C, N, output_T_dim]
        out = self.conv3(out)            # left of the equal signout shape: [1, 1, N, output_T_dim]       
